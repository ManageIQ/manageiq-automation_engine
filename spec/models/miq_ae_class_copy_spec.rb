describe MiqAeClassCopy do
  ...
  context 'copy multiple' do
    it 'classes' do
      ...
      expect(miq_ae_class_copy).to receive(:to_domain).with(domain, nil, false).exactly(ids.length).times.and_return(miq_ae_class)
      expect(miq_ae_class).to receive(:fqname).with(no_args).exactly(ids.length).times.and_return(fqname)
      expect(MiqAeClass).to receive(:find).with(an_instance_of(Integer)).exactly(ids.length).times.and_return(miq_ae_class)
      expect(MiqAeClassCopy).to receive(:new).with(anything).exactly(ids.length).times.and_return(miq_ae_class_copy)
